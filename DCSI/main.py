...
def train(loader, model, loss_model, opt, sche, epoch,logger,sim_epochs):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        target_pre, fusion_z, share_zs, viewsp_zs, p, view_class_specific_res = model(data, inc_V_ind, mode='train', sigma=args.sigma, epoch=epoch)
        # 创建一个 n x 1 的列向量，所有值为 1
        one_column = torch.ones(inc_V_ind.shape[0], 1)
        #将 matrix 和 ones_column 沿着列（维度1）拼接
        index_matrix = torch.cat((inc_V_ind, one_column.to('cuda:0')), dim=1)
        #classfy_view_loss
        view_loss=0
        for v in range(inc_V_ind.shape[1]+1):
            tmp_label= torch.zeros((inc_V_ind.shape[0],inc_V_ind.shape[1]+1), dtype=torch.float, device='cuda:0')
            tmp_label[:,v] = 1.0
            tmp_label = tmp_label * index_matrix[:,v].unsqueeze(1)
        ...
        for i in range(len(data)):
            loss_Fuse = loss_Fuse + torch.mean(torch.abs((label.mul(torch.log(p[i] + 1e-10)) \
                                          + (1-label).mul(torch.log(1 - p[i] + 1e-10))).mul(inc_L_ind)*inc_V_ind[:,i].unsqueeze(1))) 
        
        loss = loss_CL + loss_Fuse * args.alpha + view_loss * args.beta
        opt.zero_grad()
        loss.backward()
...
def test(loader, model, loss_model, epoch,logger):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        data=[v_data.to('cuda:0') for v_data in data]
        inc_V_ind = inc_V_ind.float().to('cuda:0')
        pred, fusion_z, _,_ ,_,_= model(data,inc_V_ind,mode='test',sigma=0,epoch=epoch)
        pred = pred.cpu()
...
if __name__ == '__main__':
    ...
    # Training args
    parser.add_argument('--n_z', type=int, default=1024) # here to set the dimension
